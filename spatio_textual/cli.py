...
Additional features:
- QA/Testimony segmentation (--testimony/--qa-segmentation)
- Sentiment (--sentiment rule|llm)
- Emotion (--emotion rule|llm)
- Interpretation (--interpret)
- Viz: map (GeoJSON->Folium) and co-occurrence graph export
"""
...
from .viz import to_geojson, make_map_geojson, build_cooccurrence
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="spatio-textual: DH toolkit")
    ...
    # spaCy
    p.add_argument("--spacy-model", default=os.environ.get("SPACY_MODEL", "en_core_web_sm"))
    p.add_argument("--resources-dir", default=None)
    p.add_argument("--no-entity-ruler", action="store_true")

    # Chunking (legacy path)
    ...
    # Feature toggles
    p.add_argument("--testimony", action="store_true", help="Enable testimony Q/A segmentation before annotation")
    p.add_argument("--qa-segmentation", action="store_true", help="Alias of --testimony")

    p.add_argument("--sentiment", choices=["off","rule","llm"], default="off")
    p.add_argument("--emotion", choices=["off","rule","llm"], default="off")
    p.add_argument("--interpret", action="store_true", help="Add summaries/interpretations/themes")

    # Viz
    p.add_argument("--viz", choices=["none","map","cooc"], default="none")
    p.add_argument("--viz-out", default=None)
    p.add_argument("--cooc-nodes", default="PERSON,GPE")

    return p.parse_args()


def _resolve_files(inputs: List[str], pattern: str, recursive: bool) -> List[Path]:
    files: List[Path] = []
    for item in inputs:
        p = Path(item)
        if p.exists() and p.is_file():
            ...
            files.extend(list(Path().glob(item)))
    # unique
...
def main() -> int:
    ...
    files = _resolve_files(args.input or [], args.glob, recursive)
    if not files:
        print("No files matched.", file=sys.stderr)
        return 1

    # Build spaCy annotator
    nlp = load_spacy_model(args.spacy_model, resources_dir=args.resources_dir, add_entity_ruler=(not args.no_entity_ruler))
    annotator = Annotator(nlp, resources_dir=args.resources_dir)

    # Optional analyzers
    sent = SentimentAnalyzer(backend=args.sentiment) if args.sentiment != "off" else None
    emo = EmotionAnalyzer(backend=args.emotion) if args.emotion != "off" else None

    # Iterate files
    results: List[dict] = []
    use_tqdm = args.tqdm and (tqdm is not None)
    it = tqdm(files, desc="Files") if use_tqdm else files

    for f in it:
        raw = f.read_text(encoding="utf-8", errors="ignore")

        # 1) Testimony segmentation or standard chunking
        segments: List[str]
        if args.testimony or args.qa_segmentation:
            qa_segments = segment_testimony(raw, nlp=nlp)
            segments = [s.text for s in qa_segments]
            meta = qa_segments
        elif args.no_chunk:
            segments = [raw]
            meta = None
        else:
            # use utils.split_into_segments via annotator helper
            segments = [s for s in annotator.annotate_file_chunked(f, n_segments=args.segments, include_text=True)]
            # above returns annotated dicts; adapt to run pipeline once more for extra features
            # We'll just reuse those to avoid re-annotating here.
            results.extend(segments)
            continue  # they are already annotated; skip to next file

        # 2) Annotate segments (entities only; no verbs by default)
        ann = annotator.annotate_texts([*segments], file_id=f.stem, include_text=args.include_text)

        # 3) Enrich with testimony meta (if any)
        if meta is not None:
            for r, s in zip(ann, meta):
                r.update({
                    "role": s.role,
                    "turnId": s.turn_id,
        ...
        if sent is not None:
            ...
            for r, p in zip(ann, preds):
                r["sentiment_label"] = p.get("label")
                r["sentiment_score"] = p.get("score")
        if emo is not None:
            preds = emo.predict(texts)
            for r, p in zip(ann, preds):
                r["emotion_label"] = p.get("label")
                r["emotion_score"] = p.get("score")
                if "distribution" in p:
                    r["emotion_dist"] = p["distribution"]

        # 5) Interpretation
        if args.interpret:
            ann = analyze_records(ann)

        # set file provenance
        for r in ann:
            r.setdefault("file", str(f))
            r.setdefault("fileId", f.stem)
            r.setdefault("segCount", len(ann))
        results.extend(ann)

    # 6) Viz (optional post-step)
    if args.viz == "map":
        # User must supply a geocoder in Python for now; this path only writes GeoJSON HTML if geocoded already.
        gj = to_geojson(results, geocoder=None)
        out = args.viz_out or "map.html"
        path = make_map_geojson(gj, out_html=out)
        print(json.dumps({"map": path, "features": len(gj.get("features", []))}, ensure_ascii=False))

    if args.viz == "cooc":
        labels = tuple([s.strip() for s in args.cooc_nodes.split(",") if s.strip()])
        edges = build_cooccurrence(results, nodes=labels)
        out = args.viz_out or "cooc.csv"
